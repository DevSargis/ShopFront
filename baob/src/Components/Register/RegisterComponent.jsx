...
import AlertComponent from "../ErrorComponents/AlertComponent";
...
export const RegisterComponent = () => {
  ...
  const handleSignUp = () => {
    setStatusCode(0); // Clear any previous error
    if (emailIsValid && phoneIsValid && repeatPasswordIsMatchet) {
      request("POST", "/api/v1/users/register", registerData)
      .then(data => {
        handleStatus(4, "You have registered successfully")
      }).catch(error => {
        const errorMessage = error.message || 'An error occurred.';
        handleStatus(1,errorMessage)
      });
      return;
    }
  };
  ...
  return (
    <Box className='generalRegister' >
      <AlertComponent message = {message} infoStatus={statusCode} />
